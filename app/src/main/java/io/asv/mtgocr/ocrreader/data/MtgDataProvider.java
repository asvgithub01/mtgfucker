...
public class MtgDataProvider extends DataProviderBase implements IDataProvider {
  ...
  @Override public void GetCardInfo(String name_card, final CardInfo cardInfo) {
  ...
  private void getHtmlInfoFromMtginfo(String url, final CardInfo cardInfo) {
    try {
      Ion.with(this.mContext.getApplicationContext())
          .load(url)
          .asString()
          .setCallback(new FutureCallback<String>() {
            @Override public void onCompleted(Exception e, String a) {
              try {
  ...
  private void getUrlDescFromMtgInfo(String url, final CardInfo cardinfo, final int idx,
      final int lastDesc) throws Exception {
        ...
        .setCallback(new FutureCallback<String>() {
          @Override public void onCompleted(Exception e, String a) {
            try {
              ...
              String typeCard = a.substring(ini + "<p>".length(), fin)
                  .replace("\n", " ")
                  .replaceAll("<[^>]*>", "");
              ...
              if (idx == lastDesc) sendMessage(ALL_DATA_COMPLETE, cardinfo);
            } catch (Exception e1) {
              e1.printStackTrace();
  ...
  private void getImgCardFromMtgInfo(String a, final CardInfo cardinfo) throws Exception {
    try {
      //get
      ///scans/en/mma/201.jpg
      //int ini = a.indexOf("src=\"http://partner.tcgplayer.com/x3/mchl.ashx");
      int ini = a.indexOf("TCGPHiLoContainer");
        ini = a.indexOf("</table>",ini);
      ini = a.indexOf("<img src=\"", ini);
      int fin = a.indexOf("\"", ini + "<img src=\"".length() + 1);
      String imgPath = a.substring(ini + "<img src=\"".length(), fin);
      //set
      cardinfo.setImgPath("https://magiccards.info"+imgPath);
      //send
  ...
  private void getPriceFromMtgInfo(String a, final CardInfo cardinfo) throws Exception {
    ...
    // http://store.tcgplayer.com/magic/modern-masters/arcbound-worker?partner=MAGCINFO
    int ini = a.indexOf("src=\"/tcgplayer/hl");
    int fin = a.indexOf("\"><", ini);

    String urlPriceInfo = a.substring(ini + "src=\"".length(), fin).replace("amp;", "");
    urlPriceInfo = "http://magiccards.info" + urlPriceInfo;
    Ion.with(this.mContext.getApplicationContext())
        .load(urlPriceInfo)
        .asString()
        .setCallback(new FutureCallback<String>() {
